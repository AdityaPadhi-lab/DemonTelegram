...
async def stream_send(bot_client: Client, chat_id: int, file_path: str, progress_message: Message = None):
    ...
    full_caption = (
        f"📘 **{clean_name}**\n"
        f"🪩 **Shared by:** [@AstroCollapse](https://t.me/+vRPr9SMdviY2YjM9)\n"
        f"🔆 **Join [@AstroCollapse](https://t.me/+vRPr9SMdviY2YjM9) for more premium notes & study materials!** 🔆"
        f"{ai_caption_text}"
   )
...
async def mirror_gdrive(_, message: Message):
    items = sorted(TEMP_DIR.glob("*"), key=os.path.getmtime, reverse=True)
    if not items:
        return await message.reply("⚠️ No file to upload.")
    file = items[0]
    await message.reply("☁️ Uploading to Google Drive...")
    try:
        gauth = GoogleAuth()
        ...
        f.Upload()
        await message.reply(f"✅ Uploaded to Google Drive:\n📂 `{file.name}`")
    except Exception as e:
        await message.reply(f"❌ Google Drive upload failed: {e}")
...
async def start_cmd(_, message: Message):
    ...
    markup = InlineKeyboardMarkup(
        [[InlineKeyboardButton("🔔 Channel Updates 🔔", url="https://t.me/+vRPr9SMdviY2YjM9")]]
    )
...
async def help_command(_, message: Message):
    help_text = (
        ...
        "🧠 **AI & Smart Tools**\n"
        "• `/ai_caption` — Generate an AI-based caption for media.\n"
        "• `/analyze` — Get details (size, duration, type) of the last file.\n"
        "• `/preview` — Generate video thumbnail preview.\n\n"
        ...
        "🔗 **Useful Links**\n"
        "• [Join Channel](https://t.me/+vRPr9SMdviY2YjM9)\n"
        "• Developer: @AstroCollapse\n"
    )

    markup = InlineKeyboardMarkup([
        [InlineKeyboardButton("🔔 Join Channel 🔔", url="https://t.me/+vRPr9SMdviY2YjM9")],
...
async def download_range(bot_client: Client, message: Message):
    ...
    loading = await message.reply(f"🧿\n📥 Downloading posts {start_id}–{end_id}… \n🧿")
